...
public class DataLinkUser {
    ...
    //获取上传登录时间
    public String getTime(){return LOGINTIME;}
    ...
    private boolean login_do() {
        ...
        String SQL="SELECT user.UID,user.UNAME,user.SID,user.SAFETY,user.PASSINFO,user.REGTIME," +
        ...
        try {
            ...
            while(res.next()){
                ...
                user.setRegTime(res.getString(6));
                LOGINTIME=res.getString(7);
                user.setRead(res.getInt(8));
                user.setUserOrg(res.getInt(9));
                user.setUserMent(res.getInt(10));
                login_read();
                return true;
            }
